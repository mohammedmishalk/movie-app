...
export default function MovieScreen({ route, navigation }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const url = `https://api.themoviedb.org/3/find/${movieId}?external_source=`;
        const options = {
          method: 'GET',
          headers: {
            accept: 'application/json',
            Authorization: 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiI2MzJjMmJiOWUxMDljODViMjcxM2MzZDlmNDc2NTJmYiIsInN1YiI6IjYzYzdkZGFiMTQyZWYxMDBhMDRhM2ViNyIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.5dtGxHLBSpw3rRoTHdXXMncHUcd2nbbT3KsdSqfVslk',
          },
        };
  
        const response = await fetch(url, options);
        const data = await response.json();
        setMovie(data.movie_results);
      } catch (error) {
        ...
        <View style={styles.castSubMenuContainer}>
          ...
          >
            <Text
              style={{
                ...
                color: isCastSelected ? "#0000" : "#808080",
              }}
            >
              Cast
            </Text>
          ...
          >
            <Text
              style={{
                ...styles.castSubMenuText,
                color: isCastSelected ? "#808080" : "#0000",
              }}
            >
              Crew
            </Text>
